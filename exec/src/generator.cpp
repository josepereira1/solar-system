...
void write_points_to_file_index(TAD_ARRAY_LIST points, int** positions, const char* path){
	int tam = getArraySize(points);
	list2fileWindex(points,positions,tam,path);
}
...
void design_cone(float radius, float height, int slices, int stacks, const char* patch) {
	...
	write_points_to_file_index(withoutRepeated,positions,patch);
}
