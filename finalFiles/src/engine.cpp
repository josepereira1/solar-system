...
void design(Group g){
    ...
    for(unsigned i = 0; i<os ;i++) {
        ...
        switch(op.flag){
            case 't':
                //mudar esta função
                //está assim só para não dar erro
                glTranslatef(getX(op.point[0]),getY(op.point[0]),getZ(op.point[0]));
                break;
            case 'r':
                //mudar esta função
                glRotatef(op.ang, getX(op.point[0]),getY(op.point[0]),getZ(op.point[0]));
                break;
            case 's':
                glScalef(getX(op.point[0]),getY(op.point[0]),getZ(op.point[0]));
                break;
            default:
    ...
    for(unsigned i = 0; i<fs ;i++) {
        ...
        int nt = pontos.size();
        for(unsigned i = 0; i < nt; i+=3 )
            glVertex3f(pontos[i], pontos[i+1], pontos[i+2]);
        glEnd();
        //glEnable(GL_CULL_FACE);
    }

    int fis = g.filhos.size();
    for(unsigned i = 0; i < fis ;i++)
        design(g.filhos[i]);
