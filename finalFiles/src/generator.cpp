...
//void write_points_to_file(vector<TAD_POINT> points, const char* path){
void write_points_to_file(TAD_ARRAY_LIST points, const char* path){
	int tam = points.size();
	int** positions;
	vector<TAD_POINT> withoutRepeated;
	filter(points,withoutRepeat,positions)
	list2file(withoutRepeat,positions,tam,patch);
}

void design_sphere(float radius, int slices, int stacks, const char* patch) {
	vector<TAD_POINT> points = getPointsOfSphere(radius,slices,stacks);
	write_points_to_file(points,patch);
}

void design_plane(float side, const char* patch) {
	vector<TAD_POINT> points = getPointsOfPlane(side);
	write_points_to_file(points,patch);	
}

void design_box(float x, float y, float z, int divisions, const char* patch) {
	vector<TAD_POINT> points = getPointsOfBox(x, y, z, divisions);
	write_points_to_file(points,patch);
}

void design_cone(float radius, float height, int slices, int stacks, const char* patch) {
	vector<TAD_POINT> points = getPointsOfCone(radius, height, slices, stacks);
	write_points_to_file(points,patch);
}
