...
//system calls para leitura do ficheiro
#include <fcntl.h>
#include <unistd.h>
...
// para debug
/*
static void printStr(int n, char* str) {
	for(int i=0;i<n;i++) {
		printf("%c", str[i]);
	}
	puts("");
}
static void printIndicesArray(TAD_ARRAY_LIST arr) {
	printf("\nINDICES= ");
	for(int i = 0; i < getArraySize(arr); i++){
    	printf("%d, ", *(int*) getElem(arr, i));
  	}
  	puts("\n");
}
...
TAD_ARRAY_LIST file2list(const char* path) {
	...
	char* tmp; // string temporária para converter para números 
